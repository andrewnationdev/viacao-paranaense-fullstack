...
<div class="flex flex-col gap-4">
    <span class="text-center text-2xl m-4 font-bold">{{ticket.cpf}}</span>
    <div class="bg-stone-100 text-slate-700 w-full mb-2 flex flex-col gap-2 p-4 text-xs">
        ...
        <hr/>
        <span>Tarifa:<strong></strong>{{''}}</span>
        <span>Pedágio:<strong></strong>{{''}}</span>
        <span>Taxa de Embarque:<strong>{{''}}</strong></span>
   ...
   <button
        class="flex justify-around items-center gap-4 w-full p-4 bg-teal-500 hover:bg-teal-600 active:scale-95 text-white font-bold rounded-lg text-lg transition-all shadow-md"
      >
        <Printer />
        Imprimir o Bilhete
      </button>
