<template>
  <div v-if="id_selecionado ? id_selecionado == viagem.id : true"
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex flex-col">
        ...
        <span class="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wider font-medium mt-1">
    ...
    viagem.duration
        ...
        <span class="text-xs text-blue-500 mt-[-8px] bg-white px-2 italic">🚌</span>
      </div>
    ...
    <div class="flex justify-between items-center border-t border-dashed border-gray-200 pt-4">
      <div class="flex flex-col">
        <span class="text-[10px] text-gray-400 uppercase leading-none">A partir de</span>
        <span class="text-xl font-bold text-green-500 mt-1">
          R$ {{ viagem.price.toFixed(2).replace('.', ',') }}
        </span>
      </div>
      <button v-if="!id_selecionado"
